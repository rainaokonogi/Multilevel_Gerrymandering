...
def make_24_blocks(block_size):
    ...
    # for block_size in [2, 3, 4, 5, 6, 10, 12, 15, 20, 30]:
    ...
    for r in range(24):
        for c in range(0, 24, 2):
            ...
            assignment[n1] = district
            assignment[n2] = district
            district += 1
    ...
    proposal = partial(
        recom, pop_col=pop_col, pop_target=block_size, epsilon=0.0001, node_repeats=2
    )
    ...
    for i in range(len(samples)):
        ...
        save_to_file = (
            f"/share/duchin/raina/6-12-24_grid_exp/24_block_partitions_100_steps/block_size_{block_size}/sample_{i+1}.json"
        )
