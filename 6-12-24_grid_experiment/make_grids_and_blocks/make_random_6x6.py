...
def create_dual_graph(num_r_units, map_number):
    ...
    """
    save_grid_to = (
        f"/share/duchin/raina/6-12-24_grid_exp/unit_maps/map_.jsons/med_BR_r_units_{num_r_units}_map_{map_number}.json"
    )
    os.makedirs(os.path.dirname(save_grid_to), exist_ok=True)
    
    # Create blank 12x12 grid
    unit_dual_graph = Graph.from_networkx(nx.grid_2d_graph(6,6))
    unit_dual_graph = Graph.from_networkx(nx.relabel.convert_node_labels_to_integers(unit_dual_graph, label_attribute ="old_node_index"))
    ...
    assert(d_counter == 36-num_r_units), "Something went terribly wrong. Map did not assign correct number of Democratic units."
...
def create_dual_graph_image(num_r_units, map_number):
    ...
    # )
    save_grid_map_images_to = f"/share/duchin/raina/6-12-24_grid_exp/unit_maps/map_.pngs/med_BR_r_units_{num_r_units}_map_{map_number}.png"
    ...
    # )
    unit_dual_graph_json = f"/share/duchin/raina/6-12-24_grid_exp/unit_maps/map_.jsons/med_BR_r_units_{num_r_units}_map_{map_number}.json"
