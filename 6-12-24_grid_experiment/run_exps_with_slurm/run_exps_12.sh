                    ...
                    for num_r_units in 18 21; do
                        for block_size in 1; do
                            ...
                            sbatch --job-name="12-syn-exps" \
                                ...
                                --wrap="PYTHONHASHSEED=0 uv run /share/duchin/raina/REPLICATION_REPO/6-12-24_grid_experiment/experiment_files/syn_exps_cli_12.py --assort-score $assort_score --num-r-units $num_r_units --map-number $map_number --block-size $block_size --experiment-type $experiment_type --init-part $init_part --random-seed $random_seed --burst-length 20 --total-steps 20000"
                        done
                    done
                done
            done
