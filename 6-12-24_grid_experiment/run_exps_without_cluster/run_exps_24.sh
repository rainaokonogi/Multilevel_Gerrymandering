                ...
                for num_r_units in 18 21; do
                    for block_size in 1 2 3; do
                        for party in "D" "R"; do
                            ...
                            PYTHONHASHSEED=0 uv run ${WORKING_DIRECTORY}/REPLICATION_REPO/6-12-24_grid_experiment/experiment_files/syn_exps_cli_24.py --num-r-units $num_r_units --block-size $block_size --experiment-type $experiment_type --init-part $init_part --random-seed $random_seed --burst-length 20 --total-steps 20000
                        done
                    done
                done
            done
