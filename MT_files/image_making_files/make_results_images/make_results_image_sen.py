...
counts = np.array(blockgroup_data)
# data_points = np.repeat(np.arange(len(counts)), counts.astype(int))

bin_width = 1
n = counts.sum()
print(n)

blockgroup_density = counts / (n * bin_width)
# sns.histplot(data_points, kde=True, stat="density", color="skyblue", bins=len(counts))


# Plot histograms
# plt.bar(x - 0.5, blockgroup_density, width=bin_width, align='edge', alpha=0.5, label="Block Groups", color=colors[0])
...
# Plot maxes
num = 0
with open(f"/share/duchin/raina/REPLICATION_REPO/MT_results/gerry_max_and_min_values/gerry_D_POV/A_gerry_toward_D_using_sen_data_sen.jsonl",'r') as f:
    for line in f:
...
with open("/share/duchin/raina/REPLICATION_REPO/MT_results/gerry_max_and_min_values/gerry_R_POV/A_gerry_toward_R_using_sen_data_sen.jsonl",'r') as f:
    for line in f:
        data = json.loads(line)
        info = data["Min Vals"]
        # transformed_info = [50 - x for x in info]
        counts = Counter(info)
...
plt.xticks(np.arange(0, 50, 4), fontsize=18)
plt.yticks([])
plt.xlim(0, 50)
plt.ylim(0, 0.35)
plt.ylabel("")
...
# plt.tight_layout()
plt.savefig("/share/duchin/raina/Montana_files/A_paper_images_compiled_2/MT_sen_image_no_label.png")