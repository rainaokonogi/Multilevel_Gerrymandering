...
def make_image(block_type, election):
    """
    ...
    election (str): The voter data we're using to determine Dem/Rep membership ("pres" for Pres2020, "sen" for Sen2020).
    """
    ...
    file_to_open = f"{CURRENT_WORKING_DIRECTORY}/MT_files/dual_graphs/morans_i_graphs/dem_v_rep/MT_{block_type}_dual_graph_{election}_for_moran.json"
    with open(file_to_open) as f:
        ...
        for i, node in enumerate(data["nodes"]):
            total_votes = (node[f"{election_caps}DEM"] + node[f"{election_caps}REP"])
            if total_votes != 0:
                x_values_index_to_vote_percentage[i] = node[f"{election_caps}DEM"] / total_votes
            else:
                raise ValueError( # We should have gotten rid of these nodes in pre-processing
                f"Found node with zero total votes."
            )
        ...
        plt.tight_layout()
        save_location = f"{CURRENT_WORKING_DIRECTORY}/image_replication/MT_morans_i_images//MT_dem_v_rep_{election}_{block_type}_morans_i.png"
...
def main():
    ...
    for block_type in ["vtds", "blockgroups", "tracts", "counties"]:
        for election in ["sen","pres"]:
            make_image(block_type, election)
