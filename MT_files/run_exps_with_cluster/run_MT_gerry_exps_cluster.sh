            ...
            for party in "D" "R"; do
                for block_type in "vtds" "blockgroups" "tracts"; do
                    ...
                    sbatch --job-name="MT-${block_type}-${election}-${party}--part-${init_part}-seed-${random_seed}" \
                        ...
                        --wrap="PYTHONHASHSEED=0 uv run ${WORKING_DIRECTORY}/REPLICATION_REPO/MT_files/experiment_files/MT_gerry_exp_cli.py --block-type $block_type --election $election --party $party --init-part $init_part --random-seed $random_seed --burst-length 20 --total-steps 1000000"
