...
def main():
    """Creates one image summarizing the results of the NY experiments using Pres2020 data.
    ...
    vtds_data = np.load(f"{CURRENT_WORKING_DIRECTORY}/NY_files/processed_results_data/neutral_histogram_data/neutral_pres_vtds_histogram.npy", allow_pickle=True)
    blockgroup_data = np.load(f"{CURRENT_WORKING_DIRECTORY}/NY_files/processed_results_data/neutral_histogram_data/neutral_pres_blockgroups_histogram.npy", allow_pickle=True)
    tracts_data = np.load(f"{CURRENT_WORKING_DIRECTORY}/NY_files/processed_results_data/neutral_histogram_data/neutral_pres_tracts_histogram.npy", allow_pickle=True)
    ...
    with open(f"{CURRENT_WORKING_DIRECTORY}/NY_files/processed_results_data/gerry_max_and_min_values/gerry_toward_D_using_pres_data.jsonl",'r') as f:
    ...
    with open(f"{CURRENT_WORKING_DIRECTORY}/NY_files/processed_results_data/gerry_max_and_min_values/gerry_toward_R_using_pres_data.jsonl",'r') as f:
        for i, line in enumerate(f):
            data = json.loads(line)
            block_type = data["Block type"]
            info = data["Min Vals"]
    ...
    plt.tight_layout()
    plt.savefig(Path(output_dir) / "NY_pres_results.png", dpi=600)
