...
def main():
    """Creates one image summarizing the results of the NY experiments using Sen2022 data.
    ...
    vtds_data = np.load(f"{CURRENT_WORKING_DIRECTORY}/NY_files/processed_results_data/neutral_histogram_data/neutral_sen_vtds_histogram.npy", allow_pickle=True)
    blockgroup_data = np.load(f"{CURRENT_WORKING_DIRECTORY}/NY_files/processed_results_data/neutral_histogram_data/neutral_sen_blockgroups_histogram.npy", allow_pickle=True)
    tracts_data = np.load(f"{CURRENT_WORKING_DIRECTORY}/NY_files/processed_results_data/neutral_histogram_data/neutral_sen_tracts_histogram.npy", allow_pickle=True)
    ...
    with open(f"{CURRENT_WORKING_DIRECTORY}/NY_files/processed_results_data/gerry_max_and_min_values/gerry_toward_D_using_sen_data.jsonl",'r') as f:
    ...
    with open(f"{CURRENT_WORKING_DIRECTORY}/NY_files/processed_results_data/gerry_max_and_min_values/gerry_toward_R_using_sen_data.jsonl",'r') as f:
        for i, line in enumerate(f):
            data = json.loads(line)
            block_type = data["Block type"]
            info = data["Min Vals"]
    ...
    # Plot vote share
    plt.axvline(x=35.953, color='black', linewidth=4)
    ...
    # plt.title("Comparing performance of different census units to neutral ensemble \n 2022 Senate race, New York")
    plt.tight_layout()
    plt.savefig(Path(output_dir) / "NY_sen_results.png", dpi=600)
