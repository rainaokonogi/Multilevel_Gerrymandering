...
# Neutral data
blockgroup_data = np.load("/share/duchin/raina/Montana_files/saved_histograms/NY/neutral_sen_blockgroups_histogram.npy", allow_pickle=True)
vtds_data = np.load("/share/duchin/raina/Montana_files/saved_histograms/NY/neutral_sen_vtds_histogram.npy", allow_pickle=True)
tracts_data = np.load("/share/duchin/raina/Montana_files/saved_histograms/NY/neutral_sen_tracts_histogram.npy", allow_pickle=True)


print(len(blockgroup_data))
print(len(vtds_data))
print(len(tracts_data))
...
with open("/share/duchin/raina/NY_output_ensembles/max_seats_D_revised/gerry_toward_D_using_sen_data_sen.jsonl",'r') as f:
...
for xj, i in all_points:
    plt.vlines(xj, 0, 0.25, linewidth=0.5, alpha=0.8, color=colors[i])

all_points = []

with open("/share/duchin/raina/NY_output_ensembles/max_seats_R_revised/gerry_toward_R_using_sen_data_sen.jsonl",'r') as f:
    for i, line in enumerate(f):
        data = json.loads(line)
        info = data["Max Vals"]
...
plt.tight_layout()
plt.savefig("/share/duchin/raina/Montana_files/A_paper_images_compiled_2/NY_SEN_TEST_IMAGE.png",dpi=600)