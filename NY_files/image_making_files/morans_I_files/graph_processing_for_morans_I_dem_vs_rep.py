...
# Pre-process all New York dual graphs
for block_type in ["vtds", "blockgroups", "tracts", "counties"]:
    ny_dual_graph = f"{CURRENT_WORKING_DIRECTORY}/NY_files/dual_graphs/NY_{block_type}_dual_graph.json"
    
    G = Graph.from_json(ny_dual_graph)

    G_pres = collapse_zero_vote_nodes(G, "PRES20")
    G_sen = collapse_zero_vote_nodes(G, "SEN22")

    pres_save_location = f"{CURRENT_WORKING_DIRECTORY}/NY_files/dual_graphs/dem_v_rep/morans_i_graphs/NY_{block_type}_dual_graph_pres_for_moran.json"
    sen_save_location = f"{CURRENT_WORKING_DIRECTORY}/NY_files/dual_graphs/dem_v_rep/morans_i_graphs/NY_{block_type}_dual_graph_sen_for_moran.json"
