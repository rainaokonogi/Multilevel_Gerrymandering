            ...
            for party in "D" "R"; do
                for block_type in "vtds" "blockgroups" "tracts"; do
                    ...
                    sbatch --job-name="NY-${block_type}-${election}-${party}-seed-${random_seed}-init-part-${init_part}" \
                        ...
                        --wrap="PYTHONHASHSEED=0 uv run ${WORKING_DIRECTORY}/REPLICATION_REPO/NY_files/experiment_files/NY_gerry_exp_cli.py --block-type $block_type --election $election --party $party --init-part $init_part --random-seed $random_seed --burst-length 20 --total-steps 1000000"
                done
            done
        done
    done
