    ...
    for init_part in {1..5}; do
        for block_type in "blockgroups" "vtds" "tracts"; do
            ...
            sbatch --job-name="NY-${block_type}-part-${init_part}-seed-${random_seed}" \
                ...
                --wrap="PYTHONHASHSEED=0 uv run ${WORKING_DIRECTORY}/NY_files/experiment_files/NY_neutral_exp_cli.py --block-type $block_type --init-part $init_part --random-seed $random_seed --total-steps 1000000"
        done
    done
done