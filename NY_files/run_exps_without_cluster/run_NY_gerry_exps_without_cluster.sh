        ...
        for election in "pres" "sen"; do
            for party in "D" "R"; do
                for block_type in "blockgroups" "vtds" "tracts"; do
                    ...
                    PYTHONHASHSEED=0 uv run ${WORKING_DIRECTORY}/REPLICATION_REPO/NY_files/experiment_files/NY_gerry_exp_cli.py --block-type $block_type --election $election --party $party --init-part $init_part --random-seed $random_seed --burst-length 20 --total-steps 1000000
