...
def find_maximums(run_path, out_path):
    """
    Looks at the short bursts optimization runs gerrymandered toward the blue party
    and saves, for each chain, the maximum number of seats won by the blue party in any plan.

    Args:
    run_path: path containing results folders for all experiments on one underlying grid
    out_path: file where we'll record all the maximums for experiments on underlying grid
    """
    ...
    with open(out_path, "w") as outfile:
        ...
        for block_folder in tqdm(block_folders, desc="block folders", leave=False):
            ...
            max_values = []   # will hold 1500 values
                    ...
                    with open(exp_path) as f:
                        for line in f:
                            ...
                            if seats_without_ties != 12:
                                seats_d += 0.5 * (12 - seats_without_ties)

                            # Update max number of blue seats found across the chain as necessary
                            if file_max is None or file_max < seats_d:
                                file_max = seats_d

                    if file_max is not None:
                        max_values.append(file_max)

            # Write 1500 max values per block size per underlying grid
            out_record = {
                "Block size": block_folder,
                "max_Seats_won_D_values": max_values
            }
...
def main():
    """
    Performs data analysis of finding max numbers of blue seats
    for all fixed grid experiments gerrymandered toward blue.
    """

    BASE_DIR = f"{CURRENT_WORKING_DIRECTORY}/REP_DATA/12x12_grid_results/NG/output_updaters/toward_D/"
    OUT_DIR = Path(f"{CURRENT_WORKING_DIRECTORY}/fixed_grid_experiment/processed_results_data_(replicated)/NG_maximums")
    ...
    for run_folder in tqdm(run_folders, desc="Run folders"):
        ...
        find_maximums(run_path, out_path)
