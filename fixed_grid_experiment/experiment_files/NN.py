...
def run_experiment_nn(assort_score, num_r_units, map_number, block_size, init_part, random_seed, total_steps):
    ...
    for sample in range(1,101):
        ...
        save_assignment_results_to = (
            f"{CURRENT_WORKING_DIRECTORY}/REP_DATA_(replicated)/12x12_grid_results/NN/output_ensembles/{assort_score}_BR_score_r_units_{num_r_units}_map_{map_number}/block_size_{block_size}/"
        ...
        save_updaters_results_to = (
            f"{CURRENT_WORKING_DIRECTORY}/REP_DATA_(replicated)/12x12_grid_results/NN/output_updaters/{assort_score}_BR_score_r_units_{num_r_units}_map_{map_number}/block_size_{block_size}/"
