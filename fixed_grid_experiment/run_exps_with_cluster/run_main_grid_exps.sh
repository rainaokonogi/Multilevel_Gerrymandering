                        ...
                        for block_size in 1 2 3 4 6; do
                            for party in "D" "R"; do
                                ...
                                sbatch --job-name="syn-exps" \
                                    ...
                                    --mem=2G \
                                    --time=4-00:00:00 \
                                    --error="fixed_grid_error_files/fixed_grid_error_assort_score_${assort_score}_r_${num_r_units}_map_${map_number}_block_size_${block_size}_${experiment_type}_init_part_${init_part}_seed_${random_seed}_party_${party}.log" \
                                    --output="fixed_grid_output_files/fixed_grid_output_${assort_score}_r_${num_r_units}_map_${map_number}_block_size_${block_size}_${experiment_type}_init_part_${init_part}_seed_${random_seed}_party_${party}.out" \
                                    --wrap="PYTHONHASHSEED=0 uv run ${WORKING_DIRECTORY}/fixed_grid_experiment/experiment_files/fixed_grid_exps_cli.py --assort-score $assort_score --num-r-units $num_r_units --map-number $map_number --block-size $block_size --experiment-type $experiment_type --init-part $init_part --random-seed $random_seed --party $party --burst-length 20 --total-steps 20000"
                            done
                        done
                    done
                done
