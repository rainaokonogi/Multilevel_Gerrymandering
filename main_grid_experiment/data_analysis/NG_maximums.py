...
OUT_DIR = Path(f"{CURRENT_WORKING_DIRECTORY}/main_grid_experiment/processed_results_data_(replicated)/NG_maximums")
...
def find_maximums(run_folder):
    """Performs data analysis on some of the main grid experiment results.
    Specifically, looks at the short bursts optimization runs gerrymandered toward the blue party
    and saves the maximum number of seats won by the blue party in any plan for each chain.
    """
    ...
    with open(out_path, "w") as outfile:
        ...
        for block_folder in tqdm(block_folders, desc=f"{run_folder} blocks", leave=False):
            ...
            max_values = []   # will hold 1500 numbers
            ...
            tqdm.write(f"{run_folder} | {block_folder} count = {len(max_values)}")
