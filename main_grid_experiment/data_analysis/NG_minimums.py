...
BASE_DIR = f"{CURRENT_WORKING_DIRECTORY}/REP_DATA/12x12_grid_results/NG/output_updaters/toward_R/"
OUT_DIR = Path(f"{CURRENT_WORKING_DIRECTORY}/main_grid_experiment/processed_results_data_(replicated)/NG_minimums")
...
def find_minimums(run_folder):
    """Performs data analysis on some of the main grid experiment results.
    Specifically, looks at the short bursts optimization runs gerrymandered toward the red party
    and saves the minimum number of seats won by the blue party in any plan for each chain.
    """
