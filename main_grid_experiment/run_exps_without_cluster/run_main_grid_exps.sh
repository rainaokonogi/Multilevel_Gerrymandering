                    ...
                    for num_r_units in 72 86; do
                        for block_size in 1 2 3 4 6; do
                            for party in "D" "R"; do
                                ...
                                PYTHONHASHSEED=0 uv run ${WORKING_DIRECTORY}/syn_files/syn_exps_cli.py --assort-score $assort_score --num-r-units $num_r_units --map-number $map_number --block-size $block_size --experiment-type $experiment_type --init-part $init_part --random-seed $random_seed --party $party --burst-length 20 --total-steps 20000
                            done
                        done
                    done
                done
